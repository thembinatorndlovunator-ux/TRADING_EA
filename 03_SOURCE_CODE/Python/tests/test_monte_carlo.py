...
from analysis.monte_carlo import MIN_N_RESAMPLES, main, run, run_monte_carlo
...
def test_non_positive_starting_balance_rejected():
    ...
    with pytest.raises(InsufficientSampleError):
        run_monte_carlo([1.0, 2.0], n_resamples=100, seed=1, starting_balance=0.0)
    with pytest.raises(InsufficientSampleError):
        run_monte_carlo([1.0, 2.0], n_resamples=100, seed=1, starting_balance=-500.0)
...
def test_below_minimum_n_resamples_rejected():
    ...
    with pytest.raises(ValueError):
        run_monte_carlo([1.0, 2.0], n_resamples=1, seed=1)
    assert MIN_N_RESAMPLES > 1


def test_confidence_out_of_range_raises():
    with pytest.raises(ValueError):
        run_monte_carlo([1.0, 2.0], n_resamples=100, seed=1, confidence=1.5)
...
def test_zero_variance_pnl_collapses_exactly():
    ...
    result = run_monte_carlo([10.0] * 4, n_resamples=100, seed=7, starting_balance=100.0)
    assert result.final_balance_mean == pytest.approx(140.0)
    assert result.final_balance_ci_lower == pytest.approx(140.0)
    assert result.final_balance_ci_upper == pytest.approx(140.0)
...
def test_deterministic_given_same_seed():
    ...
    a = run_monte_carlo(pnl, n_resamples=300, seed=42, starting_balance=100.0)
    b = run_monte_carlo(pnl, n_resamples=300, seed=42, starting_balance=100.0)
    assert a == b


def test_different_seed_can_differ():
    pnl = [10.0, -5.0, 20.0, -15.0, 8.0]
    a = run_monte_carlo(pnl, n_resamples=300, seed=1, starting_balance=100.0)
    b = run_monte_carlo(pnl, n_resamples=300, seed=2, starting_balance=100.0)
    assert a != b


def test_ruin_threshold_none_gives_none_prob_ruin():
    result = run_monte_carlo([10.0, -5.0], n_resamples=100, seed=1, starting_balance=100.0, ruin_threshold=None)
